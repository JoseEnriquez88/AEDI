...
void iniciarProceso(){
	...
	printf( "\n\nDNI\t\t|\tCódigo de carrera\t|\tGenero\t|\tEdad\t\n" ); 
	printf( "-----------------------------------------------------------------------------------\n" );
}
...
void procesarAlumno() {	
	...
	if(vr_alumno.genero=='m'){/*pregunta si son mujeres*/
		contMujeres++;
			if (vr_alumno.edad<25 && vr_alumno.codCarrera==29){/*si son mujeres menores a 25 y de la carrera Ing Electrónica*/
				contMujeresElectronica++;
			}
			if (vr_alumno.edad<edadMujerMenor){/*busca la menor edad de las mujeres*/
...
void finalizarProceso(){
	...
	printf("\nEl porcentaje de mujeres menores a 25 de la carrera Ing Electrónica es: %.2f%%", (float)contMujeresElectronica*100/contMujeres);
	printf("\nLa cantidad de mujeres en la Carrera Ing en Electrónica: %d", contMujeresElectronica);
